...
export default function Header() {
  ...
  const navItems = [
    ...
    {
      name: t("navigation.store", "Store"),
      href: "/store",
      icon: <ShoppingBag size={16} />,
    },
    {
      name: t("navigation.jobs", "Jobs"),
      href: "/jobs",
      icon: <Briefcase size={16} />,
    },
    {
      name: t("navigation.rules", "Rules"),
      href: "/rules",
  ...
  return (
    ...
    >
              ...
              <div className="relative flex items-center">
                <span className="text-primary font-display text-2xl font-bold animate-text-flicker">
                  <span className="text-primary">
                    {t(
                      "common.site_name_first",
                      locale === "en" ? "WEXON" : "ويكسون"
                    )}
                  </span>
                </span>
          ...
          <div className="flex items-center space-x-4 rtl:space-x-reverse">
            ...
            {!isLoading && currentUser && (
              ...
              >
                ...
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 rtl:-right-auto rtl:-left-1 w-5 h-5 bg-primary text-xs text-white rounded-full flex items-center justify-center">
                    {unreadCount}
                  </span>
                )}
              </Link>
            ...
            {!isLoading && !currentUser ? (
              ...
              <div className="relative group" id="user-menu-container">
                ...
                <div
                  ...
                  } mt-2 w-64 bg-card/95 backdrop-blur-lg border border-border rounded-lg shadow-neon overflow-hidden origin-top-right transition-all duration-300 transform ${
                    userMenuOpen
                ...
                >
                  ...
                  <div className="py-1">
                    ...
                    currentUser?.role === "owner" ? (
                      <Link
                        href="/admin"
                        className={`flex items-center px-4 py-2 text-foreground hover:text-primary hover:bg-primary/10 transition-colors ${
                    ...
                    <Link
                      href="/inbox"
                      className={`flex items-center px-4 py-2 text-foreground hover:text-primary hover:bg-primary/10 transition-colors sm:hidden ${
                        isRTL ? "flex-row-reverse" : ""
                      }`}
                    >
                      ...
                      {unreadCount > 0 && (
                        <span
                          className={`${
                            isRTL ? "mr-2" : "ml-2"
                          } px-1.5 py-0.5 bg-primary text-xs text-white rounded-full`}
                        >
                          {unreadCount}
                        </span>
                      )}
                  ...
                  <div className="py-1 border-t border-border">
                    <button
                      ...
                      className={`w-full flex items-center px-4 py-2 text-foreground hover:text-primary hover:bg-primary/10 transition-colors ${
                        isRTL ? "flex-row-reverse" : ""
                      }`}
                    >
                      <LogOut
      ...
      <div
        ...
        } absolute top-full left-4 right-4 mt-2 py-4 bg-card/95 backdrop-blur-sm rounded-lg border border-border shadow-neon`}
      >
        <nav className="flex flex-col space-y-2">
          ...
          {!currentUser && (
            <Link
              href="/login"
              className={`flex items-center px-4 py-2 text-foreground hover:bg-primary/10 hover:text-primary transition-colors ${
                isRTL ? "flex-row-reverse" : ""
              }`}
              onClick={() => setMobileMenuOpen(false)}
            >
          ...
          {!currentUser && (
            <Link
              href="/register"
              className={`flex items-center px-4 py-2 text-primary hover:bg-primary/10 transition-colors ${
                isRTL ? "flex-row-reverse" : ""
              }`}
              onClick={() => setMobileMenuOpen(false)}
            >
