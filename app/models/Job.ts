...
export const updateJob = async (id: string, jobData: Partial<Omit<Job, 'id' | 'createdAt' | 'updatedAt'>>) => {
  const { db } = await connectToDatabase();
  const jobsCollection = db.collection('jobs');
...
export const getJobs = async (filters?: { isOpen?: boolean; isFeatured?: boolean; category?: string }) => {
  const { db } = await connectToDatabase();
  const jobsCollection = db.collection('jobs');
  ...
  return jobsCollection.find(query).toArray();
};

export const createApplication = async (applicationData: Omit<JobApplication, 'id' | 'status' | 'createdAt' | 'updatedAt'>) => {
  const { db } = await connectToDatabase();
...
export const updateApplicationStatus = async (id: string, status: 'pending' | 'approved' | 'rejected') => {
  const { db } = await connectToDatabase();
  const applicationsCollection = db.collection('jobApplications');
...
export const getApplications = async (jobId?: string, status?: 'pending' | 'approved' | 'rejected') => {
  const { db } = await connectToDatabase();
  const applicationsCollection = db.collection('jobApplications');
...
export const getUserApplications = async (userId: string) => {
  ...
  return applicationsCollection.find({ userId }).sort({ createdAt: -1 }).toArray();
};

export const checkUserApplicationStatus = async (userId: string, jobId: string) => {
  const { db } = await connectToDatabase();
  const applicationsCollection = db.collection('jobApplications');
