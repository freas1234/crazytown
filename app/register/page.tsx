...
function RegisterContent() {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.password.length < 10) {
      setErrorMessage(
        t?.errorMessages?.password_min_length ||
    ...
    if (!hasUpperCase || !hasLowerCase || !hasNumbers || !hasSymbols) {
      setErrorMessage(
        t?.errorMessages?.password_requirements ||
    ...
    if (
      formData.password.toLowerCase().includes(emailPart) &&
      emailPart.length > 3
    ) {
      setErrorMessage(
        t?.errorMessages?.password_similar_email ||
          "Password cannot be similar to your email"
      );
      return false;
    }
    ...
    if (/(.)\1{2,}/.test(formData.password)) {
      setErrorMessage(
        t?.errorMessages?.password_repeated_chars ||
    ...
    try {
      ...
      if (!isDevelopment) {
        ...
        if (!captchaResult.success) {
          setRecaptchaError(
            captchaResult.message || "reCAPTCHA verification failed"
          );
          setRecaptchaToken("");
          return;
        }
                ...
                <div className="mt-2 text-xs text-gray-400">
                  ...
                  <ul className="space-y-1 text-xs">
                    ...
                    >
                      •{" "}
                      {locale === "ar"
                        ? "لا توجد أحرف متكررة أكثر من مرتين"
