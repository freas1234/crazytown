      ...
      try {
        ...
        const productPromises = parsedCart.map(item => {
          ...
          return fetch(`/api/products/${item.productId}`)
            .then(res => {
              console.log(`Product ${item.productId} response status:`, res.status);
              return res.ok ? res.json() : null;
            })
            .then(data => {
              console.log(`Product ${item.productId} data:`, data?.product || null);
              return data?.product;
            })
            .catch(err => {
        ...
        const cartWithProducts = parsedCart.map((item, index) => ({
          ...item,
  ...
  const proceedToCheckout = async () => {
    try {
      ...
      if (outOfStockItems.length > 0) {
        const itemNames = outOfStockItems.map(item => item.product?.name[locale]).join(', ');
        toast.error(`Some items are out of stock or have insufficient quantity: ${itemNames}`);
        setLoadingCheckout(false);
        return;
      }
                ...
                <div>
                  ...
                  {!loading && cart.length > 0 && (
                    <p className="text-muted-foreground mt-2">
                      {totalItems} {totalItems === 1 ? t('store.cart.item', 'item') : t('store.cart.items', 'items')}
                    </p>
                  )}
                </div>
                <Button variant="outline" size="sm" asChild className="group border-primary/30 hover:bg-primary/20 hover:text-primary">
              ...
              {loading ? (
                ...
                <Card className="max-w-4xl mx-auto border-gray-800 bg-secondary/30 backdrop-blur-sm shadow-lg overflow-hidden">
                  <CardContent className="pt-12 pb-12 text-center flex flex-col items-center justify-center min-h-[300px]">
                    ...
                    <h2 className="text-2xl font-bold text-white mb-2">{t('store.cart.empty_cart', 'Your cart is empty')}</h2>
                    <p className="text-muted-foreground mb-8 max-w-md mx-auto">{t('store.cart.empty_cart_message', 'Looks like you haven\'t added any items to your cart yet.')}</p>
                    <Button asChild size="lg" className="px-8 py-6 h-auto bg-primary hover:bg-primary/90 text-white font-medium rounded-lg transition-all hover:scale-105">
                      <Link href="/store">
                    ...
                    <Card className="border-gray-800 bg-secondary/30 backdrop-blur-sm shadow-lg overflow-hidden">
                      ...
                      <ScrollArea className="h-[calc(100vh-400px)] min-h-[300px]">
                        <CardContent className="p-0">
                          {cart.map((item) => (
                            <div key={item.productId} className="flex flex-col md:flex-row items-start gap-4 border-b border-gray-800 pb-4 mb-4 last:border-0 last:mb-0 last:pb-0">
                              ...
                              <div className="flex-1">
                                <Link href={`/store/product/${item.productId}`} className="text-white hover:text-primary transition-colors font-medium">
                                ...
                                <div className="flex items-center mt-1">
                                  {item.product?.salePrice && item.product.salePrice > 0 ? (
                                    <>
                                      <span className="text-primary font-medium">${item.product.salePrice.toFixed(2)}</span>
                                      <span className="text-gray-400 line-through text-sm ml-2">${item.product?.price.toFixed(2)}</span>
                                      {calculateDiscountPercentage(item.product.price, item.product.salePrice) && (
                                        <span className="ml-2 bg-primary/20 text-primary text-xs px-1.5 py-0.5 rounded">
                                          {calculateDiscountPercentage(item.product.price, item.product.salePrice)}% {t('store.cart.off', 'off')}
                                        </span>
                                      )}
                                    </>
                                  ) : (
                                    <span className="text-primary font-medium">${item.product?.price.toFixed(2)}</span>
                                  )}
                                  <span className="text-gray-400 text-sm ml-2">{t('store.cart.each', 'each')}</span>
                                </div>
                                
                                {item.product?.outOfStock && (
                                  <div className="mt-1 text-xs text-red-500 bg-red-500/10 px-2 py-1 rounded inline-block">
                                    {item.product.outOfStockMessage?.[locale] || t('store.out_of_stock', 'Out of stock')}
                                  </div>
                                )}
                                {!item.product?.digital && !item.product?.outOfStock && (item.product?.stock || 0) < item.quantity && (
                                  <div className="mt-1 text-xs text-yellow-500 bg-yellow-500/10 px-2 py-1 rounded inline-block">
                                    {t('store.insufficient_stock', 'Only')} {item.product?.stock} {t('store.available', 'available')}
                                  </div>
                                )}
                              </div>
                              ...
                              <div className="flex items-center gap-2 mt-2 md:mt-0">
                                <div className="flex items-center border border-gray-800 rounded-md">
                                  ...
                                  <button 
                                      onClick={() => updateQuantity(item.productId, item.quantity + 1)}
                                    className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-white transition-colors"
                                    disabled={!item.product?.digital && (item.product?.stock || 0) <= item.quantity}
                                  >
                                    +
                                  </button>
                                </div>
                    ...
                    <Card className="border-gray-800 bg-secondary/30 backdrop-blur-sm shadow-lg sticky top-24">
                      ...
                      <CardFooter className="pt-2 pb-6">
                        ...
                        >
                          ...
                          ) : t('store.cart.proceed_to_checkout', 'Proceed to Checkout')}
                        </Button>
                      </CardFooter>
                    </Card>
                  </div>
