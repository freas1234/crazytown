...
const JWT_AUTH_SECRET: string = process.env.JWT_SECRET || 'default_jwt_secret_change_this';
...
export async function generateAuthToken(user: { _id: ObjectId | string; email: string; role: string; username?: string }): Promise<string> {
  const payload = {
...
export function verifyAuthToken(token: string): UserPayload | null {
  try {
    const decoded = jwt.verify(token, JWT_AUTH_SECRET as string) as UserPayload;
    return decoded;
  } catch (error) {
