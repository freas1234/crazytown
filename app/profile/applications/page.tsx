...
export default function MyApplications() {
  ...
  const fetchApplications = async () => {
    try {
      ...
      const jobIds: string[] = Array.from(new Set(data.applications.map((app: Application) => app.jobId)));
      ...
      setError(error instanceof Error ? error.message : 'Failed to load your applications');
      setLoading(false);
    }
  };
            ...
            <div className="bg-secondary/80 border border-gray-800 rounded-lg p-8 max-w-lg mx-auto">
              <div className="text-center mb-6">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16 text-yellow-500 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m0 0l3-3m-3 3l-3-3m-2-5a4 4 0 011.5-3.2A4 4 0 0116 6.5 4 4 0 0119.5 10m-13.5 0l3 3m-3-3l-3 3" />
                </svg>
                <h2 className="text-2xl font-bold text-white mt-4">Login Required</h2>
                <p className="text-gray-400 mt-2">
                  Please login to view your job applications.
                </p>
              </div>
              <div className="text-center">
                <Link href="/login" className="inline-block px-6 py-3 bg-primary hover:bg-primary/80 text-white font-medium rounded-lg transition-colors">
                  Login
                </Link>
              </div>
            </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 gap-6">
              {applications.map((application) => {
                ...
                return (
                  <div key={application.id} className="bg-secondary/80 border border-gray-800 rounded-lg p-6">
                    ...
                    {application.status === 'approved' && (
                      <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4 text-sm">
                        <p className="text-green-400">
                          Congratulations! Your application has been approved. Our team will contact you soon with next steps.
                        </p>
                      </div>
                    )}
            ...
            <div className="bg-secondary/80 border border-gray-800 rounded-lg p-12 text-center">
              ...
              <p className="text-gray-400 mb-6">
                You haven't applied for any positions yet. Check out our open positions and submit your application.
              </p>
              <Link href="/jobs" className="inline-block px-6 py-3 bg-primary hover:bg-primary/80 text-white font-medium rounded-lg transition-colors">
